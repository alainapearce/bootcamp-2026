...
As of `r Sys.Date()`, we have had *n*=`r sum(!is.na(people_signed_in$signin_date))` attendees and presenters sign in.
Of these, *n*=`r sum(people_signed_in$signin_noregister)` attended but did not register in advance.
*N*=`r sum(people_signed_in$register_nosignin)` registered or committed to present but did not sign-in.

Here is the distribution of units.

```{r}
#| label: fig-checkins-coll
#| fig-cap: "Check-ins by college/unit."
 people_signed_in |>
